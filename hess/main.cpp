...
int main(int argc, char *argv[])
{
  ...
  auto cb_grad_func2 = [g, L, U, k, &population, &w, &W, &w_hat, &S, &F_0, &F_1, &clusters, &UB](const double* multipliers, double& f_val, double* grad) {
    f_val = 0;
  ...
  try {
    ...
    lagrangianBasedSafeFixing(F_0, F_1, clusters, W, S, LB, UB);
    for (int i = 0; i < g->nr_nodes; ++i)
    {
      for (int j = 0; j < g->nr_nodes; ++j)
      {
